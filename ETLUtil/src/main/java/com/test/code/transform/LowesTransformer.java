...
public class LowesTransformer {
	private static final char COMMA_DELIMITER = ',';
	private static final char QUOTE_CHAR = '"';
	private static final String MODE_OF_PAYMENT ="LOWES CARD";
	private static final String MODE_OF_PAYMENT_TANAYA ="TANAYA-LOWES CARD";
	...
	public static List<Expense> processData(List<Expense> expenseList){
		...
		try
		{
			...
			csvReader = new CSVReader(new FileReader(PropertiesUtil.getProperty("lowesFile")),COMMA_DELIMITER,QUOTE_CHAR,1);
			...
			System.err.println("Error file: "+PropertiesUtil.getProperty("lowesFile")+"\n Error Data: "+StringUtil.printArray(expenseDetails));
			e.printStackTrace();
		}
		try
		{
			...
			csvReader = new CSVReader(new FileReader(PropertiesUtil.getProperty("lowesFileTanaya")),COMMA_DELIMITER,QUOTE_CHAR,1);
			while((expenseDetails = csvReader.readNext())!=null)
			{
				if(!expenseDetails[4].trim().toUpperCase().contains(PAYMENT_DONE) 
						&& !expenseDetails[4].trim().toUpperCase().contains(PAYMENT_DONE1)
			...
			System.err.println("Error file: "+PropertiesUtil.getProperty("lowesFileTanaya")+"\n Error Data: "+StringUtil.printArray(expenseDetails));
			e.printStackTrace();
		}
