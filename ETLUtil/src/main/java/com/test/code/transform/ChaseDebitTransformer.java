...
public class ChaseDebitTransformer {
	...
	public static List<Expense> processData(List<Expense> expenseList,List<Earning> earningList){
		...
		try
		{
			...
			csvReader = new CSVReader(new FileReader(PropertiesUtil.getProperty("ChaseDebitFile")),COMMA_DELIMITER,QUOTE_CHAR,8);
			while((expenseDetails = csvReader.readNext())!=null)
			{
				if(!excludeExpense(expenseDetails[2].trim())) {
					...
					if(isEarning(expenseDetails[2].trim())) {
						...
						earning.setMerchant(expenseDetails[2].trim().toUpperCase());
						//earning.setEarningPlace(expenseDetails[3].trim().toUpperCase());
						earning.setAmount(StringUtil.getDouble(expenseDetails[3].trim()));
			...
			System.err.println("Error file: "+PropertiesUtil.getProperty("ChaseDebitFile")+"\n Error Data: "+StringUtil.printArray(expenseDetails));
			e.printStackTrace();
		}
		return expenseList;
	}
