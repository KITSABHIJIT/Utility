...
public class Controller {
	...
	public static List<BurlapRecord> getBurlapRecords() {
		List<BurlapRecord> craftRecordList=new ArrayList<BurlapRecord>();
		File dataFile=new File(FILE);
		if(dataFile.exists()){
			RecordContainer container;
			try {
				container = (RecordContainer)StoreRetreiveData.retrieveData(FILE);
				for(Map.Entry<String,BurlapRecord> record : (container.getBurlapRecord()).entrySet()){
					craftRecordList.add(record.getValue());
				}
				 Collections.sort(craftRecordList, new Comparator<BurlapRecord>() {
				        public int compare(BurlapRecord s1, BurlapRecord s2) {
